(function () {
  ...
  const loadedInPortrait = window.matchMedia("(orientation: portrait)").matches;

  if (unifiedPhoneTest && isPhoneLike) {
    document.documentElement.dataset.unifiedPhoneUi = "true";
    document.documentElement.dataset.phoneOrientation = loadedInPortrait
      ? "portrait"
      : "landscape";
    ...
    if (!loadedInPortrait) {
      ...
      if (viewport) {
        viewport.setAttribute("content", "width=1200, viewport-fit=cover");
      }
    }
    ...
    unifiedPhoneStyle.textContent = `
      html[data-unified-phone-ui="true"][data-phone-orientation="portrait"] body {
        overflow: hidden !important;
      }

      html[data-unified-phone-ui="true"] #phone-orientation-gate {
        display: none;
      }

      html[data-unified-phone-ui="true"][data-phone-orientation="portrait"] #phone-orientation-gate {
        position: fixed;
        z-index: 100000;
        inset: 0;
        display: flex;
      ...
      #phone-orientation-gate .orientation-phone {
        font-size: 52px;
        line-height: 1;
        transform: rotate(90deg);
      }

      #phone-orientation-gate strong {
        font-size: 24px;
        line-height: 1.15;
        letter-spacing: .02em;
      }

      #phone-orientation-gate span {
        max-width: 320px;
        color: #d9deef;
        font-size: 15px;
        line-height: 1.4;
    ...
    function ensureOrientationGate() {
      if (document.getElementById("phone-orientation-gate")) return;
      const gate = document.createElement("div");
      gate.id = "phone-orientation-gate";
      gate.setAttribute("role", "status");
      gate.innerHTML = `
        <div class="orientation-phone" aria-hidden="true">▯</div>
        <strong>Rotate to landscape</strong>
        <span>PrepFlow quizzes use the full two-page book in landscape.</span>
      `;
      document.body.appendChild(gate);
    }

    if (document.readyState === "loading") {
      document.addEventListener("DOMContentLoaded", ensureOrientationGate, { once: true });
    } else {
    ...
    const reloadForOrientation = () => {
      ...
      if (nowPortrait === loadedInPortrait) return;
      window.setTimeout(() => window.location.reload(), 120);
    };

    if (typeof orientationQuery.addEventListener === "function") {
      orientationQuery.addEventListener("change", reloadForOrientation);
    } else {
      orientationQuery.addListener(reloadForOrientation);
    }
