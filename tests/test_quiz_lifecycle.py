    ...
    def continue_from_feedback(self) -> None:
        if self.review_mode:
            ...
            if self.screen == "next-block" and self.mobile_portrait:
                self.start_next_block(self.block_end)
            return
...
def test_block_size_one_advances_to_next_first_pass_question() -> None:
...
def test_resume_pending_block_completion_has_forward_route() -> None:
    quiz = QuizLifecycle(4, 1, block_start=0, question_index=1, screen="next-block")
...
def test_review_mastery_opens_next_block_question_directly() -> None:
    quiz = QuizLifecycle(4, 1)
    quiz.answer(False)
    quiz.continue_from_feedback()
    quiz.answer(True)
