...
CONTAMINATION_PATTERNS = {
    ...
    "test-bank metadata": re.compile(
        r"\b(?:NCLEX category|cognitive level|client needs|integrated process|"
        r"content area|difficulty level|page reference|learning objective)"
        r"\s*:",
        re.IGNORECASE,
    ),
}
...
def main() -> int:
    ...
    for index, raw_question in enumerate(questions, start=1):
        ...
        for relative_path, text in iter_text(raw_question, base_path):
            ...
            # Choice labels, IDs, and answer-key values are structural data,
            # not prose. Scanning values such as "A" created false positives.
            non_prose_path = (
                relative_path.endswith(".label")
                or ".correct_answers[" in relative_path
                or relative_path.endswith(".correct_answer")
                or relative_path.endswith(".question_id")
                or relative_path.endswith(".id")
            )
